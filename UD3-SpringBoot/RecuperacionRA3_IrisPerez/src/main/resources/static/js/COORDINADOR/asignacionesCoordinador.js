...
// Mostrar nombre del usuario (coordinador) que esté usando el panel en el title
fetch("/user/datos")
    .then(r => r.json())
    .then(data => {
        document.title = "Asignaciones (" + data.nombre + ")";
    });
...
function cargarAsignaciones() {
    ...
    fetch("/coordinador/asignaciones")
        .then(r => {
...
function cargarDialogCrear() {
    ...
    // Cargar camiones disponibles (ahora desde /coordinador/camiones)
    fetch("/coordinador/camiones")
        .then(r => {
        ...
        .then(camiones => {
            ...
            return fetch("/coordinador/rutas");
        })
        .then(r => {
...
function crearAsignacion() {
    const camionId = parseInt(document.getElementById("camionCrear").value);
    const rutaId = parseInt(document.getElementById("rutaCrear").value);
    const url = "/coordinador/asignaciones";

    console.log("Creating assignment:", { camionId, rutaId }, "URL:", url);
...
function eliminarAsignacion(id) {
    ...
    fetch("/coordinador/asignaciones/" + id, {
        method: "DELETE"
    }).then((r) => {
        if (!r.ok) return r.json()
            .then(data => {
