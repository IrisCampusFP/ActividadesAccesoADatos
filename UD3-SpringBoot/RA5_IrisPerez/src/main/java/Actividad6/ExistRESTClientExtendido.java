...
public class ExistRESTClientExtendido {
    ...
    public String ejecutarXQuery(String xquery) throws Exception {
        String queryWrapped = String.format(
                "<query xmlns=\"http://exist.sourceforge.net/NS/exist\"><text><![CDATA[%s]]></text></query>",
                xquery
        );
        ...
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .header("Authorization", authHeader)
                .header("Content-Type", "application/xml")
                .POST(HttpRequest.BodyPublishers.ofString(queryWrapped))
                .build();
