...
public class ExportadorCuentaJSON {
    ...
    static String fecha = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date()); // (formato para nombre archivo)
    ...
    public static boolean exportar(Cuenta cuenta, String nombreArchivo) {
        ...
        if (crearDirectorio(CARPETA)) {
            ...
            try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(rutaCompleta), StandardCharsets.UTF_8))) {
                ...
                bw.newLine();
                bw.write(INDENTACION3 + "\"nombre\": \"" + escaparJSON(cliente.getNombre()) + "\",");
                bw.newLine();
                bw.write(INDENTACION3 + "\"DNI\": \"" + escaparJSON(cliente.getDni()) + "\",");
                bw.newLine();
                bw.write(INDENTACION3 + "\"numeroCuenta\": \"" + escaparJSON(cliente.getNumeroCuenta()) + "\"");
                bw.newLine();
                bw.write(INDENTACION2 + "},");
                bw.newLine();
